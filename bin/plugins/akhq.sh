  ...
  do
    ...
    do
      ...
      then
        ...
        then
          echo "{\"sourceType\":\"topic\",\"sourceName\":\"${AKHQ_RIGHT_TOPIC}\",\"targetType\":\"app\",\"targetName\":\"${AKHQ_ACL_I_NAME}\",\"linkName\":\"consume\",\"linkProperties\":\"\"}" >> ${OUTPUT_FILE}
        elif [ "${AKHQ_RIGHT_OPERATION=}" == "WRITE" ]
        then
          echo "{\"sourceType\":\"app\",\"sourceName\":\"${AKHQ_ACL_I_NAME}\",\"targetType\":\"topic\",\"targetName\":\"${AKHQ_RIGHT_TOPIC}\",\"linkName\":\"produce\",\"linkProperties\":\"\"}" >> ${OUTPUT_FILE}
        fi
      fi
      j=$(( j+1 ))
    done
